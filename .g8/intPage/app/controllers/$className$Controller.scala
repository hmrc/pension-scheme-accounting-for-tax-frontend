...
class $className$Controller @Inject()(override val messagesApi: MessagesApi,
  ...
  def onPageLoad(mode: Mode, srn: SchemeReferenceNumber): Action[AnyContent] = (identify andThen getData(srn) andThen requireData).async {
    implicit request =>
  ...
  def onSubmit(mode: Mode, srn: SchemeReferenceNumber): Action[AnyContent] = (identify andThen getData(srn) andThen requireData).async {
    implicit request =>
      DataRetrievals.retrieveSchemeName { schemeName =>
        form.bindFromRequest().fold(
          formWithErrors => {
            ...
            val viewModel = GenericViewModel(
              submitUrl = routes.$className$Controller.onSubmit(mode, srn).url,
              returnUrl = config.schemeDashboardUrl(any(): IdentifierRequest[_]).format(srn.id),
              schemeName = schemeName)
