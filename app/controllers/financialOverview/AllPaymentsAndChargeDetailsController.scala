...
class AllPaymentsAndChargeDetailsController @Inject()(
  ...
  def onPageLoad(srn: String, period: String, index: String,
                 paymentOrChargeType: PaymentOrChargeType): Action[AnyContent] =
  ...
  private def buildPage(
    ...
    val interestUrl = controllers.financialOverview.routes.AllPaymentsAndChargesInterestController
        .onPageLoad(srn, period, index, paymentOrChargeType).url
