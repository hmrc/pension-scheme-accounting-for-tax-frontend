      ...
      (chargeRef, interestRef) match {
        ...
        case (Some(schemeFs), Some(sourceChargeInfo)) =>
          ...
          renderer.render(
            ...
            ctx = summaryListData(
              Tmp(
                srn, pstr, period, schemeFs, schemeName, paymentOrChargeType, originalAmountUrl,
        ...
        case (_, Some(schemeFs)) =>
          schemeFs.sourceChargeRefForInterest match {
            case Some(sourceChargeRef) =>
              ...
              val seqChargeRefs = paymentsAndChargesService.chargeRefs(filteredCharges)
                .find { x =>
                  originalCharge.map(charge => getPaymentOrChargeType(charge.chargeType).toString).contains(x._1._1)
                  x._1._2.equals(period)
                } match {
                case Some(found) =>
                  found._2
                case _ => Nil
