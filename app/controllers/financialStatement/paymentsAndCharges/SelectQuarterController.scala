...
class SelectQuarterController @Inject()(config: FrontendAppConfig,
  ...
  def onPageLoad(srn: SchemeReferenceNumber, year: String, journeyType: ChargeDetailsFilter): Action[AnyContent] = (identify andThen allowAccess(Some(srn))).async { implicit request =>
  ...
  def onSubmit(srn: SchemeReferenceNumber, year: String, journeyType: ChargeDetailsFilter): Action[AnyContent] = (identify andThen allowAccess(Some(srn))).async { implicit request =>
    service.getPaymentsForJourney(request.idOrException, srn, journeyType).flatMap { paymentsCache =>
