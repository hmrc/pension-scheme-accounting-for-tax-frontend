...
class AllowAccessAction(srn: String,
  ...
  override protected def filter[A](request: DataRequest[A]): Future[Option[Result]] = {
    ...
    retrieveSuspendedFlagAndSchemeStatus(request.userAnswers) {
      ...
      case (isSuspended, _) =>
        schemeDetailsConnector.checkForAssociation(request.psaId.id, srn)(hc, implicitly, request).flatMap {
          case true => associatedPsaRedirection(srn, startDate, isSuspended, optPage, version, accessType)(request)
