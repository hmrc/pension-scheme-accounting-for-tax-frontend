...
class AFTReturnTidyService {
  ...
  private val chargeInfoC = ChargeInfo(
    ...
    isDeleted = (ua, index) =>
      (ua.get(pages.chargeC.WhichTypeOfSponsoringEmployerPage(index)),
    ...
    reinstate = (ua, index) => {
      val uaWithEmployerReinstated =
        if (ua.getOrException(pages.chargeC.WhichTypeOfSponsoringEmployerPage(index)) == SponsoringEmployerTypeIndividual) {
