...
import controllers.financialOverview.routes.{AllPaymentsAndChargeDetailsController, AllPaymentsAndChargesInterestController}
...
class AllPaymentsAndChargesService @Inject()(schemeService: SchemeService,
  ...
  private def paymentsAndChargesDetails(
    ...
    val chargeDetailsItemWithStatus: PaymentAndChargeStatus => PaymentsAndChargesDetails =
      status => PaymentsAndChargesDetails(
        ...
        status = status,
        redirectUrl = AllPaymentsAndChargeDetailsController.onPageLoad(
      ...
      case (true, true) if details.accruedInterestTotal > 0 =>
        ...
        Seq(
          ...
          PaymentsAndChargesDetails(
            ...
            redirectUrl = AllPaymentsAndChargesInterestController.onPageLoad(
              srn, period, chargeRefs.indexOf(details.chargeReference).toString, paymentOrChargeType).url,
