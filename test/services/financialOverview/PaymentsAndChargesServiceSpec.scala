...
import services.financialOverview.PaymentsAndChargesServiceSpec.srn
...
object PaymentsAndChargesServiceSpec {
  ...
  private def paymentsAndChargesTwoCharges(chargeType: SchemeFSChargeType): Seq[SchemeFSDetail] =
    Seq(
      SchemeFSDetail(
        index = 1,
        chargeReference = "AYU3494534632",
        ...
        accruedInterestTotal = BigDecimal(153.00),
        periodStartDate = Some(QUARTER_START_DATE),
        periodEndDate = Some(QUARTER_END_DATE),
      ...
      SchemeFSDetail(
        ...
        chargeType = chargeType,
        dueDate = Some(LocalDate.parse("2020-05-15")),
        totalAmount = BigDecimal(56433.00),
        outstandingAmount = 56050.08,
        stoodOverAmount = 25090.08,
        amountDue = BigDecimal(1030.05),
        accruedInterestTotal = BigDecimal(154.00),
        periodStartDate = Some(QUARTER_START_DATE),
        periodEndDate = Some(QUARTER_END_DATE),
        formBundleNumber = None,
        sourceChargeRefForInterest = Some("AYU3494534632"),
        sourceChargeIndex = Some(1),
        sourceChargeFormBundleNumber = Some("1234"),
        documentLineItemDetails = Seq(item)
      )
    )
