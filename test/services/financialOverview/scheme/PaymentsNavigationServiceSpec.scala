...
class PaymentsNavigationServiceSpec extends SpecBase with MockitoSugar with BeforeAndAfterEach with ScalaFutures {
  ...
  "navFromAFTYearsPage" must {
    "redirect to SelectQuarters page if there are multiple quarters to choose from" in {
      whenReady(paymentsNavigationService.navFromAFTYearsPage(payments(), year, srn)) {
        _ mustBe Redirect(SelectQuarterController.onPageLoad(srn, year.toString))
      }
    }

    "redirect to PaymentsAndCharges page if there is only one quarter in the given year" in {
      whenReady(paymentsNavigationService.navFromAFTYearsPage(payments(QUARTER_START_DATE, QUARTER_END_DATE), year, srn)) {
        _ mustBe Redirect(AllPaymentsAndChargesController.onPageLoad(srn, QUARTER_START_DATE.toString, AccountingForTaxCharges))
      }
    }

    "redirect to SessionExpired page if there are no quarters in the given year" in {
      whenReady(paymentsNavigationService.navFromAFTYearsPage(Nil, year, srn)) {
  ...
  "navFromPaymentsTypePage" must {
    "redirect to SelectYear page if there are multiple years to choose from" in {
      whenReady(paymentsNavigationService.navFromPaymentsTypePage(
        ...
        _ mustBe Redirect(SelectYearController.onPageLoad(srn, PensionsCharges))
      }
    }

    "redirect to navFromAFTYears method if there is only one year in the AFT category" in {
      whenReady(paymentsNavigationService.navFromPaymentsTypePage(payments(), srn, AccountingForTaxCharges)) {
        _ mustBe Redirect(SelectQuarterController.onPageLoad(srn, year.toString))
      }
    }

    "redirect to PaymentsAndCharges page if there is only one year in the selected nonAFT category" in {
      whenReady(paymentsNavigationService.navFromPaymentsTypePage(payments(charge1Type = EXCESS_RELIEF_PAID), srn, ExcessReliefPaidCharges)) {
        _ mustBe Redirect(AllPaymentsAndChargesController.onPageLoad(srn, QUARTER_START_DATE.getYear.toString, ExcessReliefPaidCharges))
      }
    }

    "redirect to SessionExpired page if there are no quarters in the given year" in {
      whenReady(paymentsNavigationService.navFromPaymentsTypePage(Nil, srn, ContractSettlementCharges)) {
        _ mustBe Redirect(controllers.routes.SessionExpiredController.onPageLoad)
      }
    }
  }
  ...
  "navFromSchemeDashboard" must {
    ...
    "redirect to PaymentsAndCharges page if there is only one quarter in the given year" in {
      whenReady(paymentsNavigationService.navFromSchemeDashboard(payments(QUARTER_START_DATE, QUARTER_END_DATE), srn)) {
        _ mustBe Redirect(AllPaymentsAndChargesController.onPageLoad(srn, QUARTER_START_DATE.toString, AccountingForTaxCharges))
      }
    }

    "redirect to SessionExpired page if there are no quarters in the given year" in {
      whenReady(paymentsNavigationService.navFromSchemeDashboard(Nil, srn)) {
        _ mustBe Redirect(controllers.routes.SessionExpiredController.onPageLoad)
      }
    }
  }
