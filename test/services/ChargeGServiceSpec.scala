...
class ChargeGServiceSpec extends SpecBase {
  ...
  val allMembers: UserAnswers = UserAnswers().set(MemberDetailsPage(0), SampleData.memberDetailsG).toOption.get
  ...
  val allMembersIncludingDeleted: UserAnswers = allMembers
    .set(MemberDetailsPage(2), SampleData.memberDetailsDeletedG).toOption.get
  ...
  def expectedAllMembers: Seq[Member] = Seq(
    ...
    expectedMember(SampleData.memberDetailsG2, 1))

  def expectedMembersIncludingDeleted: Seq[Member] = expectedAllMembers ++ Seq(
    expectedMember(SampleData.memberDetailsDeletedG, 2)
  )
