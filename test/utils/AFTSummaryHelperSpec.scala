...
class AFTSummaryHelperSpec extends SpecBase with MustMatchers with MockitoSugar with BeforeAndAfterEach {
  ...
  private val userAnswers = UserAnswers(Json.obj()).setOrException(
    ...
    pages.chargeC.TotalChargeAmountPage, BigDecimal(200.00)).setOrException(
    pages.chargeF.ChargeDetailsPage, ChargeFDetails(LocalDate.now(), BigDecimal(300.00))).setOrException(
    pages.chargeD.TotalChargeAmountPage, BigDecimal(400.00)).setOrException(
    pages.chargeA.ChargeDetailsPage, ChargeADetails(1, None, None, BigDecimal(500.00))).setOrException(
    pages.chargeB.ChargeBDetailsPage, ChargeBDetails(1, BigDecimal(600.00))).setOrException(
    pages.chargeG.TotalChargeAmountPage, BigDecimal(700.00))
  ...
  "summaryListData" must {
    ...
    "return only one row with link and others with zero amount if only one charge has data" in {
      val aftSummaryHelper = new AFTSummaryHelper
      val result = aftSummaryHelper.summaryListData(UserAnswers(Json.obj()).setOrException(
        pages.chargeE.TotalChargeAmountPage, BigDecimal(100.00)), srn, startDate)
