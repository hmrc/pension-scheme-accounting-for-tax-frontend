...
class SelectQuarterUpcomingControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers
  ...
  val templateToBeRendered = "financialStatement/paymentsAndCharges/selectUpcomingQuarter.njk"
  val formProvider = new QuartersFormProvider()
  val form: Form[Quarter] = formProvider("selectUpcomingChargesQuarter.error", quarters)

  lazy val httpPathGET: String = routes.SelectQuarterUpcomingController.onPageLoad(srn).url
  lazy val httpPathPOST: String = routes.SelectQuarterUpcomingController.onSubmit(srn).url
  ...
  override def beforeEach: Unit = {
    ...
    when(mockPaymentsAndChargesService.extractUpcomingCharges).thenReturn(_ => schemeFSResponseAftAndOTC)
  }

  "SelectQuarterUpcoming Controller" must {
    ...
    "redirect to next page when valid data is submitted" in {
      ...
      redirectLocation(result) mustBe Some(routes.PaymentsAndChargesUpcomingController.onPageLoad(srn, q22020.startDate.toString).url)
    }
