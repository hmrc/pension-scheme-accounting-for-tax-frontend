...
class SelectQuarterOverdueControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers
  ...
  val templateToBeRendered = "financialStatement/paymentsAndCharges/selectOverdueQuarter.njk"
  val formProvider = new QuartersFormProvider()
  val form: Form[Quarter] = formProvider("selectOverdueChargesQuarter.error", quarters)

  lazy val httpPathGET: String = routes.SelectQuarterOverdueController.onPageLoad(srn).url
  lazy val httpPathPOST: String = routes.SelectQuarterOverdueController.onSubmit(srn).url
  ...
  override def beforeEach: Unit = {
    ...
    when(mockPaymentsAndChargesService.getOverdueCharges(any())).thenReturn(schemeFSResponseAftAndOTC)
  }
  ...
  "SelectQuarterUpcoming Controller" must {
    ...
    "redirect to next page when valid data is submitted" in {
      ...
      redirectLocation(result) mustBe Some(routes.PaymentsAndChargesOverdueController.onPageLoad(srn, q22020.startDate.toString).url)
    }
