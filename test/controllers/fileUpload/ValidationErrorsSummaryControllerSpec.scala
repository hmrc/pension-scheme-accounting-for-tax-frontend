...
class ValidationErrorsSummaryControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers {
  ...
  private def httpPathGET: String = controllers.fileUpload.routes.ValidationErrorsSummaryController.onPageLoad(srn, startDate, accessType, versionInt, chargeType).url
