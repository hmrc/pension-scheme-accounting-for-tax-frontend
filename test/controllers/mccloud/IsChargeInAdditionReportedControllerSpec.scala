...
class IsChargeInAdditionReportedControllerSpec extends ControllerSpecBase
  ...
  private val chargeTypeDescription = Messages(s"isChargeInAdditionReported.chargeType.${ChargeTypeAnnualAllowance.toString}")
  private val form: Form[Boolean] = formProvider(messages("isChargeInAdditionReported.error.required", chargeTypeDescription))

  private def httpPathGET: String = routes.IsChargeInAdditionReportedController
    .onPageLoad(ChargeTypeAnnualAllowance, NormalMode, srn, startDate, accessType, versionInt, 0).url

  private def httpPathPOST: String = routes.IsChargeInAdditionReportedController
  ...
  "IsChargeInAdditionReported Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      ...
      val expectedJson = Json.obj(
        ...
        "chargeTypeDescription" -> Messages(s"isChargeInAdditionReported.chargeType.${ChargeTypeAnnualAllowance.toString}")
      )
