...
class CannotResumeControllerSpec extends ControllerSpecBase with MockitoSugar with NunjucksSupport
  ...
  private def getRoute: String = routes.CannotResumeController.onPageLoad(srn, startDate).url

  private def onClickRoute: String = routes.CannotResumeController.onClick(srn, startDate).url

  "Cannot resume controller" must {
