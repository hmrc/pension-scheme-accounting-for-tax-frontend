...
class AddMembersControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers {
  ...
  private def table = Json.obj(
    ...
    "rows" -> Json.arr(
      Json.arr(
        Json.obj("html" -> s"<span class=hmrc-responsive-table__heading aria-hidden=true>Member</span>first last"),
        Json.obj("html" -> s"<span class=hmrc-responsive-table__heading aria-hidden=true>National Insurance number</span>AB123456C"),
        Json.obj("html" -> s"""<span class=hmrc-responsive-table__heading aria-hidden=true>Tax due</span>${FormatHelper.formatCurrencyAmountAsString(BigDecimal(50.00))}""","classes" -> "govuk-table__header--numeric"),
        Json.obj("html" -> s"<a class= govuk-link id=member-0-view href=/manage-pension-scheme-accounting-for-tax/aa/$QUARTER_START_DATE/$accessType/$versionInt/overseas-transfer-charge/1/check-your-answers><span aria-hidden=true>View</span><span class= govuk-visually-hidden>View first last’s overseas transfer charge</span> </a>"),
        Json.obj("html" -> s"<a class= govuk-link id=member-0-remove href=/manage-pension-scheme-accounting-for-tax/aa/$QUARTER_START_DATE/$accessType/$versionInt/overseas-transfer-charge/1/remove-charge><span aria-hidden=true>Remove</span><span class= govuk-visually-hidden>Remove first last’s overseas transfer charge</span> </a>")
      ),
      Json.arr(
        Json.obj("html" -> s"<span class=hmrc-responsive-table__heading aria-hidden=true>Member</span>Joe Bloggs"),
        Json.obj("html" -> s"<span class=hmrc-responsive-table__heading aria-hidden=true>National Insurance number</span>AB123456C"),
        Json.obj("html" -> s"""<span class=hmrc-responsive-table__heading aria-hidden=true>Tax due</span>${FormatHelper.formatCurrencyAmountAsString(BigDecimal(50.00))}""","classes" -> "govuk-table__header--numeric"),
        Json.obj("html" -> s"<a class= govuk-link id=member-1-view href=/manage-pension-scheme-accounting-for-tax/aa/$QUARTER_START_DATE/$accessType/$versionInt/overseas-transfer-charge/2/check-your-answers><span aria-hidden=true>View</span><span class= govuk-visually-hidden>View Joe Bloggs’s overseas transfer charge</span> </a>"),
        Json.obj("html" -> s"<a class= govuk-link id=member-1-remove href=/manage-pension-scheme-accounting-for-tax/aa/$QUARTER_START_DATE/$accessType/$versionInt/overseas-transfer-charge/2/remove-charge><span aria-hidden=true>Remove</span><span class= govuk-visually-hidden>Remove Joe Bloggs’s overseas transfer charge</span> </a>")
      ),
      Json.arr(
        Json.obj("text" -> ""),
        Json.obj("text" -> "Total", "classes" -> "govuk-table__header--numeric"),
        Json.obj("text" -> FormatHelper.formatCurrencyAmountAsString(BigDecimal(100.00)),"classes" -> "govuk-table__header--numeric"),
        Json.obj("text" -> ""),
        Json.obj("text" -> "")
      )
    ),
  ...
  private val jsonToPassToTemplate:Form[Boolean]=>JsObject = form => Json.obj(
    ...
    "quarterStart" -> QUARTER_START_DATE.format(dateFormatterDMY),
    "quarterEnd" -> QUARTER_END_DATE.format(dateFormatterDMY),
    "table" -> table
  )
