  ...
  private def table: JsObject = Json.obj(
    ...
    "rows" -> Json.arr(
      Json.arr(
        ...
        Json.obj("html" -> s"<a class= govuk-link id=member-0-removeWithCleanup href=removelink1><span aria-hidden=true>Remove</span><span class= govuk-visually-hidden>Remove first last’s lifetime allowance charge</span> </a>", "classes" -> cssQuarterWidth)
      ),
      Json.arr(
        ...
        Json.obj("html" -> s"<a class= govuk-link id=member-1-removeWithCleanup href=removelink2><span aria-hidden=true>Remove</span><span class= govuk-visually-hidden>Remove Joe Bloggs’s lifetime allowance charge</span> </a>", "classes" -> cssQuarterWidth)
      ),
      Json.arr(
        Json.obj("text" -> ""),
