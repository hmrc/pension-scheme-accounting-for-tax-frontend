...
class AddMembersControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers {
  ...
  private def httpPathGET: String = controllers.chargeD.routes.AddMembersController.onPageLoad(srn, startDate).url
  private def httpPathPOST: String = controllers.chargeD.routes.AddMembersController.onSubmit(srn, startDate).url
  ...
  private def jsonToPassToTemplate:Form[Boolean]=>JsObject = form => Json.obj(
    ...
    "viewModel" -> GenericViewModel(
      submitUrl = controllers.chargeD.routes.AddMembersController.onSubmit(srn, startDate).url,
