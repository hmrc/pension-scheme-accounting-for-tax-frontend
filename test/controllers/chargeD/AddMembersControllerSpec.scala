...
class AddMembersControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers {
  ...
  private def table: JsObject = Json.obj(
    ...
    "rows" -> Json.arr(
      Json.arr(
        ...
        Json.obj("html" -> s"<a class= govuk-link id=member-0-view href=viewlink1><span aria-hidden=true>View</span><span class= govuk-visually-hidden>View first last’s lifetime allowance charge</span> </a>","classes" -> cssQuarterWidth),
      ...
      Json.arr(
        ...
        Json.obj("text" -> "AB123456C","classes" -> cssQuarterWidth),
        Json.obj("text" -> FormatHelper.formatCurrencyAmountAsString(BigDecimal(33.44)),"classes" -> s"$cssQuarterWidth govuk-table__header--numeric"),
        Json.obj("html" -> s"<a class= govuk-link id=member-1-view href=viewlink2><span aria-hidden=true>View</span><span class= govuk-visually-hidden>View Joe Bloggs’s lifetime allowance charge</span> </a>","classes" -> cssQuarterWidth),
  ...
  override def beforeEach: Unit = {
    ...
    when(mockMemberPaginationService
      .getMembersPaginated[ChargeDDetails](any(), any(), any(), any(), any(), any(), any(), any())(any()))
  ...
  "AddMembers Controller" must {
    "return OK and the correct view for a GET and get first page" in {
      when(mockMemberPaginationService
        .getMembersPaginated[ChargeDDetails](pageCaptor.capture(), any(), any(), any(), any(), any(), any(), any())(any()))
        .thenReturn(expectedPaginatedMembersInfo)
